...
def predict(
    ...
    #%% Create Dictionary to load cleaned data
    cleaned_data = pd.read_csv(f'{solution_directory}/{airport}_clean.csv')
    #%%
    ...
    def clean_data(pushback, etd, lamp, mfs):
        ...
        drop_cols = ['timestamp', 'forecast_timestamp', 'major_carrier', 'departure_runway_estimated_time', 'benchmark_pushback_estimated_time']
    ...
    '''
    For onehote encoding using getDummies

    #Clean Test Data and create test variable to recognize test variables
    pushback = clean_data(partial_submission_format, etd, lamp, mfs)
    pushback = pushback.assign(test=1)

    #Load my own model
    my_model = model[airport]

    #Combine training and test data, one-hot encode variables, and retrieve only test data
    airport_concat = pd.concat([cleaned_data, pushback])
    airport_encoded = onehot(airport_concat)
    airport_test = airport_encoded[airport_encoded['test']==1].drop('test', axis=1)

    #TRY comparing onehot data and deleting column not in training data
    train_data = cleaned_data.drop('test', axis=1)
    train_data = onehot(train_data)
    extra_columns = airport_test.columns.difference(train_data.columns)
    airport_test = airport_test.drop(columns=extra_columns)


    #This portion is to handle the empty partial_prediction_format for predictions
        #Add one row to the test set
    first_row = airport_encoded[airport_encoded['test']==0].drop('test', axis=1)

    #TRY: need to drop the extra column in this as well
    first_row = first_row.drop(columns=extra_columns)

    airport_test = pd.concat([airport_test, first_row.iloc[[0]]], ignore_index=True, sort=False)

    #Format one row to add to empty partial_prediction_format
    last_row = first_row.assign(timestamp = '2021-10-31 11:11:00', airport = airport)
    last_row['timestamp'] = pd.to_datetime(last_row['timestamp'])
    last_row = last_row[['gufi', 'timestamp', 'airport', 'minutes_until_pushback']].iloc[0]
    ...
    #Add one row to empty partial_prediction_format 
    prediction = pd.concat([prediction, last_row.to_frame().T], ignore_index=True, sort=False)


    #Add predictions to partial_prediction_format
    prediction['minutes_until_pushback'] = y_pred

    #remove last row from dataframe, regardless of size
